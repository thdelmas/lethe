...
- **Unified Auto-Wipe Policy** (v1.2) — SOURCE MERGED, NOT YET SHIPPING. The chokepoint (`AutoWipePolicy.executeWipe` → `DPM.wipeData`), the Device Admin receiver (`LetheDeviceAdmin`), the first-boot Device Owner promotion script, and the new init service are on `main` (commits `e2b3407` → `f45f0b9`). However, `apply-overlays.sh` step 11 still echoes "LETHE agent — deferred to v1.1, not packaged" and produces no Android.mk / system_app integration — so none of this is in the built image. Verified empirically against a flashed build of commit `b0ae2b4`: `pm path org.osmosis.lethe.agent` returns empty, `/system/etc/init/` lacks `init.lethe-agent.rc`, no `persist.lethe.autowipe.*` set. Tracked as issue [#143](https://github.com/thdelmas/lethe/issues/143). Until that ships, the design below (iPhone-style failed-unlock wipe via `setMaximumFailedPasswordsForWipe`, DPM-wipe-bypassing-the-neverallow, etc.) is accurate at the source level but inactive on-device.
