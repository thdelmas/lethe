...
# Stage Filament prebuilts + the mascot GLB into the app dir (step 11
# adjunct). The GLB is a 12MB binary kept out of git; re-extract from
# history when absent — acfd45e:static/mascot-taproot.glb is the dark
# cracked model (HEAD taproot is a different model — do not use; see
# docs/design/mascot-sprites.md).
lethe_stage_filament() {
    local script_dir="$1" app_dest="$2"
    local glb="$script_dir/prebuilt/filament/mascot.glb"
    if [ ! -f "$glb" ]; then
        git -C "$script_dir" show acfd45e:static/mascot-taproot.glb > "$glb" || {
            echo "  -> ERROR: cannot extract mascot.glb from git history" >&2
            return 1
        }
    fi
    mkdir -p "$app_dest/assets" "$app_dest/prebuilt/filament/arm64"
    cp "$script_dir/prebuilt/filament"/*.jar "$app_dest/prebuilt/filament/"
    cp "$script_dir/prebuilt/filament/arm64"/*.so "$app_dest/prebuilt/filament/arm64/"
    cp "$glb" "$app_dest/assets/mascot.glb"
    echo "  -> Filament prebuilts + mascot.glb staged."
}
...
lethe_generate_app_buildfile() {
    ...
    case "$props_target" in
        *vendor/cm/*)
            # No arm64 Filament prebuilts on cm-14.1 (armv7, API 25):
            # strip the live-3d path so the Java tree still compiles.
            rm -f "$app_dest/java/org/osmosis/lethe/FilamentMascotView.java"
            rm -rf "$app_dest/prebuilt" "$app_dest/assets/mascot.glb"
            cat > "$app_dest/java/org/osmosis/lethe/MascotViews.java" <<'LETHE_CM_MV'
package org.osmosis.lethe;

import android.content.Context;
import android.view.View;

/** cm-14.1 variant (generated by overlay-helpers.sh): no Filament. */
final class MascotViews {
    private MascotViews() {}

    static View create(Context context) {
        return SpriteMascotView.available(context)
            ? new SpriteMascotView(context) : new MascotView(context);
    }
...
android_app {
    ...
    srcs: ["java/**/*.java"],
    resource_dirs: ["res"],
    // mascot.glb ships uncompressed (-0) so gltfio can read it whole.
    asset_dirs: ["assets"],
    aaptflags: ["-0", ".glb"],
    ...
    // lethe-filament-jars + jni_libs: live mascot render (FilamentMascotView).
    static_libs: ["zxing-core", "lethe-filament-jars"],
    jni_libs: ["libfilament-jni", "libgltfio-jni"],
...
// Filament 1.51.0 (vendored from Maven AARs — see ~/Lethe/prebuilt/filament).
java_import {
    name: "lethe-filament-jars",
    jars: [
        "prebuilt/filament/filament-android.jar",
        "prebuilt/filament/gltfio-android.jar",
    ],
    sdk_version: "current",
}

cc_prebuilt_library_shared {
    name: "libfilament-jni",
    arch: { arm64: { srcs: ["prebuilt/filament/arm64/libfilament-jni.so"] } },
    compile_multilib: "64",
    strip: { none: true },
...
cc_prebuilt_library_shared {
    name: "libgltfio-jni",
    arch: { arm64: { srcs: ["prebuilt/filament/arm64/libgltfio-jni.so"] } },
    compile_multilib: "64",
    strip: { none: true },
    check_elf_files: false,
}
